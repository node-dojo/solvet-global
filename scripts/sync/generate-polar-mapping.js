...
try {
  ...
  console.error('\nUsage: POLAR_API_TOKEN=xxx POLAR_ORG_ID=xxx node scripts/generate-polar-mapping.js');
  process.exit(1);
}
...
async function main() {
  console.log('🔍 Fetching products and checkout links from Polar...\n');
  ...
  if (!fs.existsSync(checkoutLinksPath)) {
    console.error('❌ Error: checkout-links.json not found!');
  ...
  for (const product of allProducts.result.items) {
    ...
    if (!checkoutUrlMap.has(product.id)) {
      console.log(`⏭️  Skipping ${product.name} (no checkout link)`);
      skippedCount++;
      continue;
    }
  ...
  if (skippedCount > 0) {
    console.log(`\n⚠️  Skipped ${skippedCount} products without checkout links\n`);
  }
...
`;
  ...
  console.log(`✅ Mapping file generated: ${outputPath}\n`);
