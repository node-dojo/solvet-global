...
- **SOLVET System V1/** - Parent folder (local workspace, NOT a git repo)
  - Contains: `no3d-tools-site/`, `solvet-asset-management-addon/` (formerly `send-no3ds-export utility/`), `schemas/`, `scripts/`, `templates/`, `plan docs/`
...
- These should be in separate repositories for clarity
...
├── solvet-asset-management-addon/  # New Git repo 4 - Internal export add-on
│   ├── .git/
│   └── [addon files from solvet-asset-management-addon/]
├── no3d-tools-addon/               # New Git repo 5 - Customer asset library add-on
│   ├── .git/
│   └── [customer-facing add-on files]
...
### 2.3 Create solvet-asset-management-addon Repository

**Purpose:** Internal SOLVET Asset Management Add-on (exports assets TO the library)

**Steps:**
1. Create GitHub repository: `github.com/[org]/solvet-asset-management-addon`
2. Create folder and initialize git:
   ```bash
   mkdir solvet-asset-management-addon
   cd solvet-asset-management-addon
   git init
   git remote add origin https://github.com/[org]/solvet-asset-management-addon.git
   ```
3. Move files:
   - `solvet-asset-management-addon/*` (formerly `send-no3ds-export utility/*`) → `solvet-asset-management-addon/`
   - Preserve existing add-on structure
4. Create `solvet-asset-management-addon/README.md`:
   - Describe purpose: Internal SOLVET Asset Management Add-on (part of SOLVET workflow)
   - Document installation and usage
   - Link to asset database repo
   - Note: This is for internal use, not a customer-facing product
5. Initial commit and push:
   ```bash
   git add .
   git commit -m "Initial commit: SOLVET Asset Management Add-on (internal export tool)"
   git branch -M main
   git push -u origin main
   ```

### 2.4 Create no3d-tools-addon Repository

**Purpose:** Customer-facing NO3D Tools Asset Library add-on (imports/consumes assets FROM the library)
...
2. Create folder and initialize git:
   ...
   ```
3. This is a NEW add-on to be built (not moving files from existing location)
4. Create `no3d-tools-addon/README.md`:
   - Describe purpose: Customer-facing NO3D Tools Asset Library add-on (product offering)
   - Document installation and usage
   - Link to asset database repo
   - Note: This is the customer-facing product, separate from the internal export add-on
5. Initial commit and push:
   ```bash
   git add .
   git commit -m "Initial commit: NO3D Tools Asset Library (customer-facing add-on)"
   git branch -M main
   git push -u origin main
   ```
...
- Internal export add-on code (`solvet-asset-management-addon/`)
- Customer asset library add-on code (`no3d-tools-addon/`)
- Schema references
...
- `solvet-asset-management-addon/.git/` - New git repo (internal export add-on)
- `solvet-asset-management-addon/README.md` - Repo documentation
- `no3d-tools-addon/.git/` - New git repo (customer asset library add-on)
...
- `no3d-tools-site/*` → `no3d-tools-website/`
- `solvet-asset-management-addon/*` (formerly `send-no3ds-export utility/*`) → `solvet-asset-management-addon/`
- Customer asset library add-on to be created in `no3d-tools-addon/` (new, not moving files)
...
- **Contents:** Files from `solvet-asset-management-addon/` (internal export add-on)

### no3d-tools-addon
- **GitHub repo:** `github.com/[org]/no3d-tools-addon`
- **Purpose:** Customer-facing NO3D Tools Asset Library add-on
- **Note:** This is a NEW add-on to be built, not moving files from existing location
